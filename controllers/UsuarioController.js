...
class UsuarioController {
  ...
  store(req, res, next){
    ...
    if(!nome || !email || !password || !loja)
      return res.status(422).json({ errors: "Preencha todos os campos de cadastro."})
  ...
  update(req, res, next) {
    ...
    Usuario.findById(req.payload.id).then((usuario => {
      if(!usuario) return res.status(401).json({ errors: "Usuario nao encontrado"});
  ...
  completeRecovery(req, res, next){
    ...
    if(!token || !password) 
      return res.render(
        "recovery/store", { error: "Preencha novamente com a sua nova Senha", succes: null, token: token }
      );
