...
const orderCart = (cart = null) => {
  ...
  const _cart = cart.map((item) => {
    const novaAltura = Math.min(item.alturaCm, item.profundidadeCm, item.larguraCm);
    const novoComprimento = Math.max(item.alturaCm, item.profundidadeCm, item.larguraCm);
    const _temp = [item.alturaCm, item.profundidadeCm, item.larguraCm].sort((a, b) => a < b);
